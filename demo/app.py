...
# 修改密码路由
@app.route('/api/editpasswd', methods=['POST'])
@login_required
def editpasswd():
    username = request.json.get('username')
    ...
    try:
        ...
        return jsonify({'message': '无效的密码编码', 'timestamp': time.time() * 1000, 'code': 500, 'status': 'error'}),
    ...
    if user:
        ...
        db.commit()
        return jsonify({'message': '修改成功', 'timestamp': time.time() * 10000, 'code': 200, 'status': 'success'})
    else:
        return jsonify({'message': '用户未找到', 'timestamp': time.time() * 10000, 'code': 404, 'status': 'error'})
...
if __name__ == '__main__':
    app.run(port=5888,host='0.0.0.0')
