...
def get_raw_request(flow: http.HTTPFlow, pretty_host = None) -> str:
    raw = assemble_request(flow.request).decode('utf-8')
    if pretty_host:
        raw = raw.replace(flow.request.host, pretty_host)
    return raw

def get_raw_response(flow: http.HTTPFlow, pretty_host = None) -> str:
    raw = assemble_response(flow.response).decode('utf-8')
    return raw
